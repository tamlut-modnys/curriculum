...
When we are working with the set library door `in`, the same pattern is occurring. This is a door that takes a set as a sample (input), and its arms build gates that let you work on that input set.
